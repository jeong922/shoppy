...
export default function Cart() {
  ...
  const { data: products } = useQuery(['carts'], () => repository.getCart(uid));
  const tatalPrice = products.reduce(
    (acc, cur) => acc + parseInt(cur.price) * cur.quantity,
    0
  );

  const deliveryCharge =
    products.length === 0 ? 0 : tatalPrice > 30000 ? 0 : 3000;
    ...
    <div className='flex flex-col items-center justify-center max-w-3xl px-8 pb-10 mx-auto mt-9 sm:px-3'>
      ...
      <section className='flex items-center justify-around w-full px-3 py-8 mt-8 border-y-2 border-neutral-200'>
        <div className='text-center'>
          <p className='text-lg font-semibold'>{replacePrice(tatalPrice)}</p>
          <p className='text-sm text-neutral-600'>상품금액</p>
        </div>
        <TbPlus className='text-xl' />
        <div className='text-center'>
          <p className='text-lg font-semibold'>
            {replacePrice(deliveryCharge)}
          </p>
        ...
        <div className='text-center'>
          <p className='text-lg font-semibold'>
            {replacePrice(tatalPrice + deliveryCharge)}
          </p>
